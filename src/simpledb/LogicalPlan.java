...
public class LogicalPlan
{
			...
			throws ParsingException
	{
		...
		LogicalFilterNode lf = new LogicalFilterNode(table, field, p,
				constantValue);
			...
			throws ParsingException
	{
		...
		if (table1.equals(table2))
			throw new ParsingException(
					"Cannot join on two fields from same table");
		LogicalJoinNode lj = new LogicalJoinNode(table1, table2, joinField1,
				joinField2, pred);
		System.out.println("Added join between " + joinField1 + " and "
				+ joinField2);
		joins.addElement(lj);
	 ...
	 *             tables added via {@link #addScan}
	 */
	public void addJoin(String joinField1, DbIterator joinField2,
			Predicate.Op pred) throws ParsingException
	{
		joinField1 = disambiguateName(joinField1);

		String table1 = joinField1.split("[.]")[0];

		LogicalSubplanJoinNode lj = new LogicalSubplanJoinNode(table1,
				joinField1, joinField2, pred);
	 ...
	 * 
	 * @return A fully qualified name of the form table.name. If the name
	...
	String disambiguateName(String name) throws ParsingException
	{
		...
		while (tableIt.hasNext())
		{
			...
			try
			{
				TupleDesc td = Database.getCatalog().getDbFile(table.t)
						.getTupleDesc();
	...
	public DbIterator physicalPlan(TransactionId t,
			HashMap<String, TableStats> baseTableStats, boolean explain)
			throws ParsingException
	{
		Iterator<LogicalScanNode> tableIt = tables.iterator();
		...
		while (tableIt.hasNext())
		{
			...
			try
			{
				ss = new SeqScan(t, Database.getCatalog().getDbFile(table.t)
						.getId(), table.alias);
			} catch (NoSuchElementException e)
			{
		...
		while (filterIt.hasNext())
		{
			...
			{
				throw new ParsingException(
						"Unknown field in filter expression " + lf.f);
			}
			...
			try
			{
				p = new Predicate(subplan.getTupleDesc().nameToId(
						disambiguateName(lf.f)), lf.p, f);
			} catch (NoSuchElementException e)
			{
			...
			double sel = s.estimateSelectivity(
					subplan.getTupleDesc().nameToId(disambiguateName(lf.f)),
					lf.p, f);
		...
		while (joinIt.hasNext())
		{
			...
			if (!isSubqueryJoin)
			{
				...
				for (java.util.Map.Entry<String, String> s : equivMap
						.entrySet())
				{
		...
		for (int i = 0; i < selectList.size(); i++)
		{
			...
			if (si.aggOp != null)
			{
				...
				{
					throw new ParsingException("Unknown field " + si.fname
							+ " in SELECT list");
				}
			...
			else if (hasAgg)
			{
				...
				{
					throw new ParsingException("Field " + si.fname
				...
				{
					throw new ParsingException("Unknown field " + groupByField
		...
		if (hasAgg)
		{
			...
			try
			{
				aggNode = new Aggregate(node,
						...
						groupByField == null ? Aggregator.NO_GROUPING : td
								.nameToId(disambiguateName(groupByField)),
						getAggOp(aggOp));
			} catch (NoSuchElementException e)
			{
		...
		if (hasOrderBy)
		{
			node = new OrderBy(node.getTupleDesc().nameToId(
					disambiguateName(oByField)), oByAsc, node);
		}

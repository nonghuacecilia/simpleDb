...
public class HeapFileEncoder
{
	 ...
	 */
	public static void convert(ArrayList<ArrayList<Integer>> tuples,
			File outFile, int npagebytes, int numFields) throws IOException
	{
		...
		for (ArrayList<Integer> tuple : tuples)
		{
			...
			for (Integer field : tuple)
			{
				...
				{
					throw new RuntimeException("Tuple has more than "
							+ numFields + " fields: ("
			...
			int numFields, Type[] typeAr) throws IOException
	{
		...
		int nheaderbytes = (nrecords / 8);// header部分所占的byte
		if (nheaderbytes * 8 < nrecords)
			nheaderbytes++; // ceiling
		int nheaderbits = nheaderbytes * 8;// header部分所占的bit
		...
		while (!done)
		{
			...
			if (c == ',' || c == '\n' || c == '\r')
			{
				...
				else if (typeAr[fieldNo] == Type.STRING_TYPE)
				{
					...
					pageStream.writeInt(s.length());// 先写入String所占的长度
					pageStream.writeBytes(s);
					while (overflow-- > 0)
						// 不足128时用0填充
						pageStream.write((byte) 0);
				}
				// 将filed写入之后，curpos需要清零，使buf[curpos++]给下一次循环使用
				curpos = 0;
				// '\r'与','都表示field。换行符说明record++，field从0开始。
				if (c == '\n')
					fieldNo = 0;
				else
					fieldNo++;
			...
			// page to disk.
			if (recordcount >= nrecords || done && recordcount > 0 || done
					&& npages == 0)
			{
				int i = 0;
				byte headerbyte = 0;

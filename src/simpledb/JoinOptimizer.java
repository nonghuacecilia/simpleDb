...
public class JoinOptimizer
{
	...
	public DbIterator instantiateJoin(LogicalJoinNode lj, DbIterator plan1,
			DbIterator plan2, HashMap<String, TableStats> stats)
			...
			double cost1, double cost2)
	{
		...
		else
		{
			...
			// HINT: You may need to use the variable "j" if you implemented a
			// join
	 ...
	 * 
	 * @param stats
	 *            table stats for all of the base tables (see
	 *            {@link #orderJoins})
	 * @param filterSelectivities
			...
			double bestCostSoFar, PlanCache pc) throws ParsingException
	{
		...
		Set<LogicalJoinNode> news = (Set<LogicalJoinNode>) ((HashSet) joinSet)
				.clone();
		news.remove(j);
		...
		{ // base case -- both are base relations
			...
			t1card = stats.get(j.t1).estimateTableCardinality(
					filterSelectivities.get(j.t1));
			leftPkey = isPkey(j.t1, j.f1);

			t2cost = j.t2 == null ? 0 : stats.get(j.t2).estimateScanCost();
			t2card = j.t2 == null ? 0 : stats.get(j.t2)
					.estimateTableCardinality(filterSelectivities.get(j.t2));
		...
		else
		{
			...
			if (doesJoin(prevBest, j.t1))
			{ // j.t1 is in prevBest
				t2cost = j.t2 == null ? 0 : stats.get(j.t2).estimateScanCost();
				t2card = j.t2 == null ? 0
						: stats.get(j.t2).estimateTableCardinality(
								filterSelectivities.get(j.t2));
				rightPkey = j.t2 == null ? false : isPkey(j.t2, j.f2);
			}
			else if (doesJoin(prevBest, j.t2))
			{ // j.t2 is in prevbest (both shouldn't be)
				t2cost = stats.get(j.t1).estimateScanCost();
				t2card = stats.get(j.t1).estimateTableCardinality(
						filterSelectivities.get(j.t1));
	...
	private boolean hasPkey(Vector<LogicalJoinNode> joinlist)
	{
		for (LogicalJoinNode j : joinlist)
		{
			if (isPkey(j.t1, j.f1) || (j.t2 != null && isPkey(j.t2, j.f2)))
				return true;
		}
		return false;
			...
			HashMap<String, Double> selectivities)
	{
		...
		for (LogicalJoinNode j : js)
		{
			...
			if (n == null)
			{ // never seen this table before
				n = new DefaultMutableTreeNode(j.t1
						+ " (Cost = "
						+ stats.get(j.t1).estimateScanCost()
						+ ", card = "
						+ stats.get(j.t1).estimateTableCardinality(
								selectivities.get(j.t1)) + ")");
				root.add(n);
			}
			else
			{
			...
			{ // never seen this table before
				...
				n = new DefaultMutableTreeNode(j.t2 == null ? "Subplan" : (j.t2
						+ " (Cost = "
						+ stats.get(j.t2).estimateScanCost()
						+ ", card = "
						+ stats.get(j.t2).estimateTableCardinality(
								selectivities.get(j.t1)) + ")"));
				root.add(n);

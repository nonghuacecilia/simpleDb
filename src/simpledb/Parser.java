...
public class Parser
{
	...
	static void processExpression(TransactionId tid, ZExpression wx,
			LogicalPlan lp) throws simpledb.ParsingException
	{
		...
		else
		{
			...
			{ // join node
				...
				{ // right op is a nested query
					try
					{
						lp.addJoin(tab1field,
								parseQuery(tid, (ZQuery) ops.elementAt(1)), op);
					} catch (IOException e)
					{
	...
	public static LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q)
	...
	public static DbIterator parseQuery(TransactionId tid, ZQuery q)
			throws IOException, Zql.ParseException, simpledb.ParsingException
	{
		return parseQueryLogicalPlan(tid, q).physicalPlan(tid, statsMap,
				explain);
	}

	static Transaction curtrans = null;

	public static void handleQueryStatement(ZQuery s)
	...
	public static void handleInsertStatement(ZInsert s)
			throws TransactionAbortedException, DbException, IOException,
			simpledb.ParsingException, Zql.ParseException
	{
		int id;
		...
		Query sdbq = new Query(new Insert(curtrans.getId(), newTups, id),
				curtrans.getId());
	...
	public static void handleDeleteStatement(ZDelete s)
			throws TransactionAbortedException, DbException, IOException,
			simpledb.ParsingException, Zql.ParseException
	{
		int id;
		...
		Query sdbq = new Query(new Delete(curtrans.getId(), lp.physicalPlan(
				curtrans.getId(), statsMap, false)), curtrans.getId());
		// XXX print field names
		sdbq.start();
	...
	public static void handleTransactStatement(ZTransactStmt s)
			throws TransactionAbortedException, DbException, IOException,
			simpledb.ParsingException, Zql.ParseException
	{
	...
	public static LogicalPlan generateLogicalPlan(TransactionId tid, String s)
	...
	static final String[] SQL_COMMANDS = { "select", "from", "where",

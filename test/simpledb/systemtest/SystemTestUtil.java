...
{
	public static final TupleDesc SINGLE_INT_DESCRIPTOR = new TupleDesc(
	...
	{
		return createRandomHeapFile(columns, rows, MAX_RAND_VALUE,
	...
	{
		return createRandomHeapFile(columns, rows, MAX_RAND_VALUE,
				columnSpecification, tuples, colPrefix);
	}
	...
	public static void matchTuples(DbFile f, TransactionId tid,
			List<ArrayList<Integer>> tuples) throws DbException,
	...
	public static void matchTuples(DbIterator iterator,
			List<ArrayList<Integer>> tuples) throws DbException,
			TransactionAbortedException, IOException
	{
...
//		System.out.println("-----------------------");
		...
		while (iterator.hasNext())
		{
			...
			Debug.log("scanned tuple: %s (%s)", t, isExpected ? "expected"
					: "not expected");
			if (!isExpected)
			{
		...
		if (!copy.isEmpty())
		{
			...
			for (ArrayList<Integer> t : copy)
			{
				...
				{
					msg += "[" + (copy.size() - MAX_TUPLES_OUTPUT)
							+ " more tuples]";
					break;
				}

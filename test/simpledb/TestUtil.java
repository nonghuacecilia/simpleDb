...
public class TestUtil
{
	...
	public static TupleIterator createTupleList(int width, int[] tupdata)
	{
		...
		TupleIterator result = new TupleIterator(Utility.getTupleDesc(width),
				tuplist);
		result.open();
		return result;
	}
	...
	public static boolean compareTuples(Tuple t1, Tuple t2)
	{
		...
		{
			if (!(t1.getTupleDesc().getType(i).equals(t2.getTupleDesc()
					.getType(i))))
				return false;
			...
			throws DbException, TransactionAbortedException
	{
		...
		while (expected.hasNext())
		{
			...
			if (!matched)
			{
				throw new RuntimeException("expected tuple not found: "
						+ expectedTup);
			}
		}
	}
	...
	public static abstract class CreateHeapFile
	{
		...
		protected void setUp() throws Exception
		{
			...
			{
				Database.reset();
				empty = Utility.createEmptyHeapFile(
						emptyFile.getAbsolutePath(), 2);
			} catch (IOException e)
			{
				throw new RuntimeException(e);
			}
